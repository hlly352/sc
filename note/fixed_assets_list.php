...
$sqldate = " AND `sc_fixed_assets`.`dodate` BETWEEN '$sdate' AND '$edate'";
//查询所有的事项
$sql_thing = "SELECT `sc_category`.`id`,`sc_category`.`cate_name` FROM `sc_fixed_assets` INNER JOIN `sc_category` ON `sc_category`.`id` = `sc_fixed_assets`.`cateid` WHERE `sc_category`.`status` = '1'";
...
if($_GET['submit']){
    ...
    if($cateid){
        $sqlwhere .= " AND `sc_fixed_assets`.`cateid` = '$cateid'";
    }
    if($source){
    	$sqlwhere .= " AND `sc_fixed_assets`.`source` = '$source'";
    }
...
//查询固定资产
$sql = "SELECT `sc_fixed_assets`.`cateid`,`sc_fixed_assets`.`name`,`sc_fixed_assets`.`specification`,`sc_fixed_assets`.`source`,`sc_fixed_assets`.`fixedid`,`sc_category`.`cate_name`,`sc_fixed_assets`.`amount`,`sc_fixed_assets`.`dodate`,`sc_fixed_assets`.`remark` FROM  `sc_fixed_assets` INNER JOIN `sc_category` ON `sc_fixed_assets`.`cateid` = `sc_category`.`id` WHERE `sc_fixed_assets`.`status` = '1' $sqlwhere $sqldate ";
//计算总金额
...
$sqllist = $sql." ORDER BY `sc_fixed_assets`.`dodate` ASC".$page->limitsql;
  ...
  <form action="" method="get">
    <table id="search">
      <tr>
        ...
        <th>来源：</th>
        <td>
          <select name="source" class=input_txt>
          		<option value="">所有</option>
          		<?php
          			foreach($array_fixed_source as $ks=>$source){
          				echo '<option value="'.$ks.'">'.$source.'</option>';
          			}
          		?>
          </select>
        </td>
        ...
        <td>
          ...
          <input type="button" value="添加" onfocus="window.location.href='fixed_assets_add.php?action=add'" class="button" />
        </td>
      </tr>
    </table>
  ...
  <form action="systemdo.php" name="system_list" method="post">
    ...
    ?>
      <tr>
        ...
        <td><?php echo $array_fixed_source[$row['source']] ?></td>
