...
//查询普通事件和重要事件
$sql_item = "SELECT `id`,`cate_code`,`cate_name` FROM `sc_category` WHERE `pid` = (SELECT `id` FROM `sc_category` WHERE `cate_code` = 'GDZC')";
?>
...
$(function(){
  ...
  $('.add_new_item').live('click',function(){
      ...
      var new_item = '<tr class="items date_'+item_num+'">        <th>项目：'+(item_num + 1)+'</th>        <td>        <select name="cate_id[]" class="input_txt item_id">     <option value="">请选择</option>         <?php
              ...
              ?>          </select>         </td>   <th>名称：</th>        <td>        	<input type="text" name="name[]" class="input_txt" />        </td>    <th>规格：</th>        <td>        	<input type="text" name="specification[]" class="input_txt">        </td>               <th>金额：</th>        <td>         <input type="text" name="amount[]" class="input_txt"   />         </td>   <th>来源:</th>        <td>        	<select name="source[]" class="input_txt">        		<?php
        			foreach($array_fixed_source as $ks=>$vs){
        				echo '<option value="'.$ks.'">'.$vs.'</option>';
        			}
        		?>        	</select>        </td>                <th>获取日期：</th><td>  <input type="text" name="date[]" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\',isShowClear:false,readOnly:true})" value="<?php echo date('Y-m-d'); ?>" class="input_txt" /><th>备注：</th><td><input type="text" name="remark[]" class="input_txt"/></td> </td>   <td>      <input type="button" value="添加项目" class="button add_new_item"> <input type="button" value="删除项目" class="button" id="remove_item">        </td>      </tr>';
...
<div id="table_sheet">
  <?php if($action == "add"){ ?>
  <h4>添加固定资产</h4>
  <form action="fixed_assets_do.php" name="system" method="post" enctype="multipart/form-data">
    <table>
      <tr class="items date_0">
        <th>项目：1</th>
        <td>
          <select name="cate_id[]" class="input_txt item_id">
        ...
        <th>规格：</th>
        <td>
        	<input type="text" name="specification[]" class="input_txt">
        </td>
        <th>金额：</th>
        <td>
