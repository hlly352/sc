...
//查询所有的事项
$sql_thing = "SELECT `sc_category`.`cate_code`,`sc_category`.`cate_name`,`sc_category`.`id` FROM `sc_finance_info` INNER JOIN `sc_category` ON `sc_category`.`id` = `sc_finance_info`.`cateid` WHERE `sc_category`.`status` = '1' AND `sc_finance_info`.`type` = 'I' GROUP BY `sc_category`.`id`";
$result_thing = $db->query($sql_thing);
if($_GET['submit']){
    $thingid = trim($_GET['thingid']);
    if($thingid){
    	$sqlwhere = " AND `sc_finance_info`.`thingid` = '$thingid'";
	}
...
//查询收入记录
$sql = "SELECT `sc_finance_info`.`cateid`,`sc_finance_info`.`financeid`,`sc_category`.`cate_name`,`sc_finance_info`.`amount`,`sc_finance_info`.`date`,`sc_finance_info`.`remark` FROM  `sc_finance_info` INNER JOIN `sc_category` ON `sc_finance_info`.`cateid` = `sc_category`.`id` WHERE `sc_finance_info`.`status` = '1' AND `sc_finance_info`.`type` = 'I' $sqlwhere $sqldate ";
  ...
  <form action="" method="get">
    <table id="search">
      <tr>
        <th>事项：</th>
        <td>
           <select name="thingid" class="input_txt">
        ...
        <td>
          ...
          <input type="button" value="添加" onfocus="window.location.href='income_add.php?action=add'" class="button" />
        </td>
      </tr>
    </table>
