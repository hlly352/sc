...
//读取系统确认人
$sql_system_employee = "SELECT `db_system_employee`.`employeeid` FROM `db_system_employee` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_system_employee`.`employeeid` WHERE `db_system_employee`.`systemid` = '$systemid' AND `db_system_employee`.`isconfirm` = 1 AND `db_employee`.`employee_status` = 1 AND `db_employee`.`account_status` = 1";
...
<div id="table_list">
  ...
  <table>
    <caption>
    <?php echo '【'.$array_system_type[$array_system['system_type']].'】'.'-'.$array_system['system_name']; ?>-确认人设置
