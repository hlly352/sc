...
//读取系统成员
$sql_system_employee = "SELECT `db_system_employee`.`employeeid` FROM `db_system_employee` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_system_employee`.`employeeid` WHERE `db_system_employee`.`systemid` = '$systemid' AND `db_employee`.`employee_status` = 1 AND `db_employee`.`account_status` = 1";
...
<div id="table_list">
  ...
  if($result_system->num_rows){
	  ...
	  //读取部门
	  $sql_dept = "SELECT `deptid`,`dept_name` FROM `db_department` WHERE `dept_status` = 1 AND `deptid` IN (SELECT `deptid` FROM `db_employee` WHERE `account_status` = 1 GROUP BY `deptid` HAVING COUNT(*) > 0) ORDER BY `dept_order` ASC,`deptid` ASC";
	  $result_dept = $db->query($sql_dept);
  ?>
  <table>
    <caption>
    <?php echo '【'.$array_system_type[$array_system['system_type']].'-'.$array_system['system_name'].'】'; ?>-成员设置
    ...
    if($result_dept->num_rows){
		...
		$sql_employee = "SELECT GROUP_CONCAT(CONCAT(`employeeid`,'#',`employee_name`) ORDER BY `employeeid` ASC SEPARATOR '/') AS `employee_info`,`deptid` FROM `db_employee` WHERE `employee_status` = 1 AND `account_status` = 1 GROUP BY `deptid`";
		...
		while($row_dept = $result_dept->fetch_assoc()){
			...
			if(array_key_exists($deptid,$array_employee)){
				...
				foreach($array_employee_date as $employee_info){
					...
					$employee_checkbox .= " <input type=\"checkbox\" name=\"employeeid\" value=\"".$employeeid."\"".$checked."/> "."<span id=\"".$employeeid."\"".$span_style.">".$employee_name."</span>";
				}
			}
	?>
    <tr>
