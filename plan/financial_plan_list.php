...
//计算各项资金金额
//计算总结余
$sql_surplus = "SELECT SUM(IF(`type` = 'I',`amount`,CONCAT('-',`amount`))) FROM `sc_finance_info` WHERE `status` = '1'";
$result_surplus = $db->query($sql_surplus);
if($result_surplus->num_rows){
  $surplus = $result_surplus->fetch_row()[0];
}
//总收入金额
$sql_income_amount = "SELECT SUM(`amount`) FROM `sc_finance_info` WHERE `type` = 'I' AND `status` = '1' $sqldate";

$result_income_amount = $db->query($sql_income_amount);
if($result_income_amount->num_rows){
	$income_amount = $result_income_amount->fetch_row()[0];
}
//总支出金额
$sql_expend_amount = "SELECT SUM(`amount`) FROM `sc_finance_info` WHERE `type` = 'O' AND `status` = '1' $sqldate";

$result_expend_amount = $db->query($sql_expend_amount);
if($result_expend_amount->num_rows){
  $expend_amount = $result_expend_amount->fetch_row()[0];
}
...
//查询财务计划
$sql = "SELECT `sc_plan_data`.`note`,`sc_plan_data`.`planid`,`sc_plan_data`.`categoryid`,`sc_plan_data`.`start_date`,`sc_plan_data`.`end_date`,`sc_plan_data`.`status`,`sc_plan_data`.`dotime`,count(`sc_plan_info`.`infoid`) AS `count` FROM `sc_plan_data` LEFT JOIN `sc_plan_info` ON `sc_plan_data`.`planid` = `sc_plan_info`.`planid` WHERE `sc_plan_data`.`status` = '1' AND `type` = 'F' GROUP BY `sc_plan_data`.`planid`";
$result = $db->query($sql);
  ...
  <form action="memory_task_do.php" name="system_list" method="post">
    <table>
      ...
      <tr>
        ...
        <td><a href="financial_plan_list_info.php?planid=<?php echo $planid; ?>"><img src="../images/system_ico/info.png" width="25"/></a></td>
      </tr>
      <?php } ?>
    </table>
    <div id="checkall">
