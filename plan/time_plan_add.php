...
$(function(){
  ...
  //添加新阶段
  $('.add_new_cate').live('click',function(){
      //查看一共有多少阶段
      var cate_num = $('.cate_id').size();
      $('#remove_cate').remove();
      var prev_time = $(this).parent().prev().prev().prev().prev().prev().children().val();

      var new_cate = '<tr class="cates date_'+cate_num+'">        <th>计划：'+(cate_num + 1)+'</th>        <td>        <select name="cateid[]" class="input_txt cate_id">     <option value="">请选择</option>         <?php
              ...
              ?>          </select>         </td>       <th>用时：</th>        <td>         <input type="text" name="amount[]" class="input_txt"   />H</td>    <th>重要事项：</th><td><input type="text" name="thing[]" class="input_txt" /><th>备注：</th><td><input type="text" name="remark[]" class="input_txt"/></td> </td>   <td>      <input type="button" value="添加阶段" class="button add_new_cate"> <input type="button" value="删除阶段" class="button" id="remove_cate">        </td>      </tr>';
      $('.cates:last').after(new_cate);
      $(this).remove();
  })
  //删除阶段
  $('#remove_cate').live('click',function(){
      var cate_num = $('.cate_id').size();
      $('.date_'+(cate_num - 1)).remove();
      //添加按钮
      var add_cate = '<input type="button" value="添加阶段" class="button add_new_cate">';
      var remove_cate = ' <input type="button"  value="删除阶段" class="button" id="remove_cate" />';
      var last_cate = $('.cates:last').children('td:last');
      last_cate.append(add_cate);
      //项目数大于1时添加删除按钮
      if(cate_num > 2){
        last_cate.append(remove_cate);
      }
  })
  //提交时判断项目名称和目标是否填写
...
<div id="table_sheet">
  <?php if($action == "add"){ ?>
  <h4>添加时间计划</h4>
  <form action="time_plan_do.php" name="system" method="post" enctype="multipart/form-data">
    <table>
       <tr>
      	<th>计划类型：</th>
      	<td>
      ...
      <tr class="cates date_0">
        <th>计划：1</th>
        <td>
          <select name="cateid[]" class="input_txt cate_id">
        ...
        <th>用时：</th>
        <td>
          <input type="text" name="amount[]" value="" class="input_txt" />H
        </td>
        <th>重要事项：</th>
        <td>
        ...
        <td>
          <input type="button" value="添加阶段" class="button add_new_cate">
        </td>
      </tr>
      <tr>
      	<th>备注：</th>
