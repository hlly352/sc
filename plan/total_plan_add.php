...
<div id="table_sheet">
  <?php if($action == "add"){ ?>
  <h4>记忆规则添加</h4>
  <form action="memory_rule_do.php" name="system" method="post" enctype="multipart/form-data">
    <table>
      <tr>
        <th>计划名称：</th>
        <td colspan="3">
      ...
      <tr class="new_rule">
        <th>目标：1</th>
        <td><input type="text" name="plan_content[]" id="system_name" class="input_txt" />
        </td>
        <th>计划时间1：</th>
        <td>
        <input type="text" name="plan_date[]" value="<?php echo date('Y-m-d') ?>" class="input_txt" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false,readOnly:true})"/>
