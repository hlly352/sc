...
if($result_total->num_rows){
  ...
  $arr_real_cateid = array();
	while($row_total = $result_total->fetch_assoc()){
		$arr_plan = array($row_total['cate_name'].$row_total['amount'].'元',floatval($row_total['plan_precent']));
		$array_plan_amount[]  = $arr_plan;
		
		$array_cate_name[] = $row_total['cate_name'];
		$arr_plan_data[] = intval($row_total['amount']);
    $cur_cateid = $row_total['id'];
    $sdate = $row_total['start_date'];
    $edate = $row_total['end_date'];
    //查询实际的支出金额
$sql_real_amount = "SELECT `parent_category`.`cate_name`,`sc_category`.`pid`,SUM(`sc_finance_info`.`amount`) AS `real_amount`,ROUND((SUM(`sc_finance_info`.`amount`)/(SELECT SUM(`amount`) FROM `sc_finance_info` WHERE `type` = 'O' AND `date` BETWEEN '$sdate' AND '$edate')) * 100,2) AS `real_precent` FROM `sc_finance_info` INNER JOIN `sc_category` ON `sc_finance_info`.`cateid` = `sc_category`.`id` INNER JOIN `sc_category` AS `parent_category` ON `sc_category`.`pid` = `parent_category`.`id` WHERE `sc_finance_info`.`type` = 'O' AND `sc_finance_info`.`cateid` IN(SELECT `id` FROM `sc_category` WHERE `pid` = '$cur_cateid') AND `sc_finance_info`.`date` BETWEEN '$sdate' AND '$edate' GROUP BY `sc_category`.`pid`";
...
if($result_real_amount->num_rows){
    while($row_real_amount = $result_real_amount->fetch_assoc()){
      $arr_real = array($row_real_amount['cate_name'].$row_real_amount['real_amount'].'元',floatval($row_real_amount['real_precent']));
      $array_real_amount[]  = $arr_real;
      $arr_real_data[] = floatval($row_real_amount['real_amount']);
      $array_sub_real[$row_real_amount['pid']] =  floatval($row_real_amount['real_amount']);
    }
}else{
  ...
  <form action="memory_task_do.php" name="system_list" method="post">
    <table>
      ...
      <tr>
        ...
        <td><?php echo $array_sub_real[$id]; ?></td>   
        <td>
          <a href="/note/expend_list.php?submit=查询&itemid=<?php echo $id ?>">
