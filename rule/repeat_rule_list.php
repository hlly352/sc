...
if($_GET['submit']){
	...
	$sqlwhere = " AND `sc_repeat_rule`.`rule_name` LIKE '%$rule_name%'";
}
$sql = "SELECT `sc_repeat_rule`.`ruleid`,`sc_repeat_rule`.`rule_name`,COUNT(`sc_repeat_rule_info`.`infoid`) AS `count`,GROUP_CONCAT(`sc_repeat_rule_info`.`info` ORDER BY `order` ASC SEPARATOR '-') AS `info`,SUM(`sc_repeat_rule_info`.`info`) AS `sum`,`sc_repeat_rule`.`rule_status`,`sc_repeat_rule`.`type` FROM `sc_repeat_rule` INNER JOIN `sc_repeat_rule_info` ON `sc_repeat_rule`.`ruleid` = `sc_repeat_rule_info`.`ruleid` WHERE `sc_repeat_rule`.`rule_status` = '1' $sqlwhere GROUP BY `sc_repeat_rule`.`ruleid` ";
$result = $db->query($sql);
$pages = new page($result->num_rows,12);
$sqllist = $sql . " ORDER BY `sc_repeat_rule`.`dotime` DESC,`sc_repeat_rule`.`ruleid` ASC" . $pages->limitsql;
