...
//获取支出的主类
$sql_expend_cate = "SELECT `id`,`cate_code`,`cate_name` FROM `sc_category` WHERE `pid` = (SELECT `id` FROM `sc_category` WHERE `cate_code` = 'ZC')";
...
$(function(){
    $('#add').live('click',function(){
        ...
        var add_info = '<tr class="new_rule">        <th>'+info_num+'</th>        <td>            <select name="expend_cate[]" class="input_txt">                <?php 
                  $result_expend_cate = $db->query($sql_expend_cate);
                    if($result_expend_cate->num_rows){
                        while($row_expend_cate = $result_expend_cate->fetch_assoc()){
                          echo '<option value="'.$row_expend_cate['id'].'">'.$row_expend_cate['cate_code'].'-'.$row_expend_cate['cate_name'].'</option>';
                        }
                    }
                ?>            </select>        </td><th>比例：</th><td><input type="text" name="proportion[]" class="input_txt"/><button type="button" id="add" class="button">添加</button>   <button type="button" id="rem" class="button">删除</button></td> </tr>';
        $(this).next().remove();
        $(this).remove();
...
<div id="table_sheet">
  <?php if($action == "add"){ ?>
  <h4>支出规则添加</h4>
  <form action="expend_do.php" name="system" method="post" enctype="multipart/form-data">
    <table>
      <tr>
        <th>规则名称：</th>
        <td colspan="3">
      ...
      <tr class="new_rule">
        <th>规则详情：1</th>
        <td>
            <select name="expend_cate[]" class="input_txt">
                <?php 
                    $result_expend_cate = $db->query($sql_expend_cate);
                    if($result_expend_cate->num_rows){
                        while($row_expend_cate = $result_expend_cate->fetch_assoc()){
        ...
        <th>比例：</th>
        <td>
          <input type="text" name="proportion[]" class="input_txt" />
        <button type="button" id="add" class="button">添加</button>
        </td>
      </tr>
      <tr id="remark">
        <th>备注：</th>
        <td colspan="3"><input type="text" name="remark" class="input_txt" />
          <span class="tag"> *必填</span></td>
      </tr>
      <tr>
        <th colspan="2">&nbsp;</th>
        <td colspan="2"><input type="submit" name="submit" id="submit" value="确定"class="button" />
