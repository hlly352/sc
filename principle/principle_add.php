...
//查询所有的原则类型
$sql_type = "SELECT `id`,`cate_code`,`cate_name` FROM `sc_category` WHERE `pid` = (SELECT `id` FROM `sc_category` WHERE `cate_code` = 'YZ')";
...
<div id="table_sheet">
  <?php if($action == "add"){ ?>
  <h4>原则添加</h4>
  <form action="principle_do.php" name="system" method="post" enctype="multipart/form-data">
    <table>
      <tr class="new_rule">
        <th width="20%">原则类型：</th>
        <td width="80%">
