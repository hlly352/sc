<div id="header">
  <h4>清单 <em>V1.0 BY Hillion</em><em style="display: none;">xdq</em></h4>
</div>
<div id="menu">
  <ul>
    <!-- <li class="menulevel"><a href="/system_config/">帮助</a></li> -->
    <li class="menulevel"><a href="list_add.php?action=add">添加清单</a>
    </li>
    <li class="menulevel"><a href="list_list.php">清单列表</a>
    </li>
    <li class="menulevel"><a href="/myjtl/">内网首页</a></li>
  </ul>
  <span>
    <?php echo $_SESSION['employee_info']['employee_name']; ?>
    <a href="../passport/logout.php">退出</a>
  </span>
  <div class="clear"></div>
</div>
